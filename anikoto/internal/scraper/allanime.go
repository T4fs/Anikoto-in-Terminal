...
func defaultCachePath() string {
	if localAppData := os.Getenv("LOCALAPPDATA"); localAppData != "" {
		return filepath.Join(localAppData, "anikoto", "cache.db")
	}
	if xdg := os.Getenv("XDG_CACHE_HOME"); xdg != "" {
		return filepath.Join(xdg, "anikoto", "cache.db")
	}
	home, err := os.UserHomeDir()
	if err == nil {
		return filepath.Join(home, ".cache", "anikoto", "cache.db")
	}
	return filepath.Join(os.TempDir(), "anikoto-cache.db")
}
